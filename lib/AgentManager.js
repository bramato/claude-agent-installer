...
`;
    ...
    installedAgents.forEach(agentName => {
      ...
      const displayName = agentName.replace(/^installer\./, '').replace(/\./g, ' ').replace(/\b\w/g, l => l.toUpperCase());
      
      instructions += `### 🔧 **${displayName}**\n`;
      instructions += `**Nome agente:** \`${agentName.replace(/^installer\./, '')}\`\n\n`;
      instructions += `**Quando usare:** ${agentInstructions}\n\n`;
      instructions += `**Invocazione:** Usa il Task tool con \`subagent_type: "${agentName.replace(/^installer\./, '')}"\`\n\n`;
      instructions += `---\n\n`;
    });
...
`;
  ...
  async updateClaudeInstructions(installedAgents) {
    const claudeFilePath = path.join(this.claudeDir, 'CLAUDE.md');
